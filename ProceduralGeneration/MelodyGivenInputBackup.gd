...
extends Node
class_name MelodyGivenInputBackup
...
class WaveFunction:
	...
	func set_up():
		...
		transition_matrix.apply_tension(probabilities.tension, note_info)
		transition_matrix.apply_step(probabilities.step, note_info)
	
	func create_transition_matrix():
		transition_matrix = Matrix.new()
		columns = []
		...
		transition_matrix.create(len(columns), columns)
				
		for y in range(0, len(columns)):
			for z in range(0, len(columns)):
				transition_matrix.set_element(y, z, search_pair(columns[y], columns[z]))
	...
	func choose_note(curr_tile : Dictionary):
		var row = transition_matrix.get_index_of_row(curr_tile.pitch)
		var note = columns[sample_matrix(row)]
		for x in range(len(tiles)):
			if tiles[x].pitch == note:
				return tiles[x].duplicate()
		
	func sample_matrix(row : int):
		var probability_scale = []
		var transition_row = transition_matrix.get_matrix()[row]
